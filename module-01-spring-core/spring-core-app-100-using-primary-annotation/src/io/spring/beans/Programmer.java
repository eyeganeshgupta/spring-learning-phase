...
💡 Using @Primary
--------------------
    1️⃣ In Spring, the @Primary annotation is used to indicate that a specific bean should be given preference when there are multiple beans of the same type available for ‘autowiring’.
    2️⃣ When we have multiple beans of the same type, and we don’t use @Qualifier to specify which one should be injected, then Spring will automatically choose the bean marked with @Primary.
    3️⃣ We also can use @Primary with other Spring annotations besides @Bean, such as @Component, @Service, @Repository or @Controller.
*/
